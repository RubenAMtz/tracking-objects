...
int main(int argc, char** argv) {
	...
	/////////////////
	cap >> prueba;
	prueba.copyTo(prueba);
	/// Convert it to gray
	cvtColor(prueba, src_gray, CV_BGR2GRAY);
	...
	HoughCircles(src_gray, circles, CV_HOUGH_GRADIENT, 1, src_gray.rows / 8, 200, 100, 0, 500);
	...
	for (size_t i = 0; i < circles.size(); i++)
	{
		...
		// circle outline
		circle(prueba, center, radius, Scalar(0, 0, 255), 3, 8, 0);
	}

	/// Show your results
	namedWindow("Hough Circle Transform Demo", WINDOW_AUTOSIZE);
	imshow("Hough Circle Transform Demo", prueba);
	/////////////////
